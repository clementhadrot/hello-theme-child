...
// --- Réglages perfs ---
if (!defined('DNC_USAGE_TRANSIENT_TTL')) define('DNC_USAGE_TRANSIENT_TTL', 12 * HOUR_IN_SECONDS);
...
function dnc_build_usage_tooltip(array $b) {
    ...
    $lines[] = "- Metas de post (ACF/Elementor, galleries…) : {$by['postmeta']}";
...
function dnc_is_attachment_missing( $att_id ) {
    ...
    // Cas 2 : pièce jointe avec URL distante (guid ou meta)
    $url = wp_get_attachment_url($att_id);
    if ( ! $url ) return true;

    $uploads = wp_get_upload_dir();
    $is_local_upload = ( strpos($url, $uploads['baseurl']) === 0 );

    if ($is_local_upload) {
        // Si WP n’a pas de chemin local mais l’URL est sous /uploads, on tente une HEAD rapide
        $resp = wp_remote_head($url, ['timeout' => 2]);
        if ( is_wp_error($resp) ) return true;
        $code = (int) wp_remote_retrieve_response_code($resp);
        return ($code >= 400);
    } else {
        // Ressource externe : HEAD rapide
        $resp = wp_remote_head($url, ['timeout' => 2]);
        if ( is_wp_error($resp) ) return true;
        $code = (int) wp_remote_retrieve_response_code($resp);
        return ($code >= 400);
...
function dnc_get_image_usage_breakdown( $att_id, $force_refresh = false ) {
    ...
    $sql_postmeta = $wpdb->prepare("
        ...
        WHERE
          ...
          OR pm.meta_value LIKE %s  /* sérialisé: \"123\" */
...
add_action('wp_ajax_dnc_delete_orphan', function () {
    ...
    if ($deleted) {
        // On purge le cache d’usage associé au passage
...
function dnc_ajax_list_valid($page, $pp, $search, $onlyMissingAlt){
    ...
    foreach ($ids as $id) {
        ...
        echo '<br /><label>Légende : </label><input type="text" class="legende-field" value="'. dnc_escape_attr_soft($legende) .'">';
...
function dnc_ajax_list_unused($page, $pp, $search, $onlyMissingAlt){
    ...
    foreach ($ids as $id) {
        ...
        echo '<td>'. ( $is_orphan ? '<span class="status-badge" style="background:#ffe0e0;border:1px solid #e99;padding:2px 6px;border-radius:4px;">Orpheline</span>' : '<span class="status-badge" style="background:#eef;border:1px solid #99c;padding:2px 6px;border-radius:4px;">Non utilisée</span>' ) .'</td>';
...
function render_alt_editor_page() {
    ...
    <div class="wrap">
        ...
        <h2 class="nav-tab-wrapper" style="margin-top:10px;">
            ...
            <a href="#" class="nav-tab" data-tab="unused">🧹 Images non utilisées</a>
        </h2>
    ...
    jQuery(function($){
        ...
        // ---- Pagination clic
        $('#valid-pagination, #orphans-pagination').on('click', '.page-link', function(e){
            e.preventDefault();
            const p = parseInt($(this).data('page'),10);
            const wrap = $(this).closest('.tab-pagination').attr('id');
            const tab = (wrap==='valid-pagination')?'valid':'orphans';
            state.page[tab] = p;
            loadTabPage(tab);
        });
        ...
        $('#filter-missing-alt').on('change', function(){
            ...
            state.page.valid = 1;
            if (state.activeTab==='valid') loadTabPage('valid');
        });
        ...
        $('#dnc-recalc-page').on('click', function(e){
            ...
            const $scope = (tab==='valid') ? $('#media-editor-table tbody') : $('#orphans-table tbody');
            const $rows = $scope.find('tr');
            if (!$rows.length) return;

            $btn.prop('disabled',true).text('Recalcul en cours…');

            let idx=0, total=$rows.length;
            const tick = () => {
                if (idx>=total){
                    $btn.prop('disabled',false).text('↻ Tout recalculer (page)');
                    if (tab==='valid') loadTabPage('valid'); // pour rafraîchir tooltips & éventuels changements
                    return;
                }
                ...
                if (tab==='valid' && id){
                    ...
                    }).always(()=>{ idx++; setTimeout(tick, 40); });
                } else {
                    idx++; setTimeout(tick, 20);
                }
            };
            tick();
        });
